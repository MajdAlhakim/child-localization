...
export async function apiDelete<T>(path: string): Promise<T> {
  const { baseUrl, apiKey } = await getSettings();
  const res = await axios.delete<T>(`${baseUrl}${path}`, {
    headers: { 'X-API-Key': apiKey },
    timeout: 10000,
  });
  return res.data;
...
export const apApi = {
  ...
  place:  (ap: PlaceApBody) => apiPost<{ status: string }>('/api/v1/venue/ap', ap),
  deleteAll: ()     => apiDelete<{ status: string }>('/api/v1/venue/aps'),
  deleteOne: (bssid: string) => apiDelete<{ status: string }>(`/api/v1/venue/ap/${encodeURIComponent(bssid)}`),
};
