# CLAUDE.md — Child Indoor Localization System

**Scope:** This workspace only
**Loaded by:** Claude Code (automatic, every session)
...
From `tasks.json`, identify:
- Which tasks are done by others (read-only to you)
...
## 3. Task Update Protocol (Mandatory — After Every Completed Task)
...
docker compose up -d --build       # detached mode (production)
docker compose ps                  # check status
docker compose logs backend --tail=50
docker compose down
```

**Tests (local, no Docker needed — uses SQLite in-memory):**
```bash
pip install -r backend/requirements.txt pytest pytest-asyncio httpx aiosqlite
pytest tests/ --tb=short -q        # all tests
pytest tests/test_ekf.py --tb=short -q   # single file
...
## 6. Architecture Constraints (Locked — Do Not Revisit)
...
| Server hosting | Cloud VM (GCP) with public IP via trakn.duckdns.org | Hosting on university network |

---

## 7. Mathematical Constants (Do Not Modify)
...
**publish_position() interface (Person C → Person D boundary):**
```python
...
## 10. Performance Requirements (Non-Negotiable)
...
- Tests use SQLite in-memory (no PostgreSQL needed for local runs).
