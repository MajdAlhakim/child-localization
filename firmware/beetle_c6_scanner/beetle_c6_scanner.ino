...
//   3. Send results as a single JSON line to tag Beetle C6 over UART
//
...
//
// UART wiring:
//   Beetle ESP32-C6 GPIO16 (TX) → XIAO tag Beetle C6 GPIO20 (D7 / RX)
//   Beetle ESP32-C6 GND         → XIAO tag Beetle C6 GND
//   (RX not wired — Beetle never receives data from tag Beetle C6)
//
...
// Serial1 = UART TX to tag Beetle C6 on GPIO16

#include <WiFi.h>

// ── Config
// ────────────────────────────────────────────────────────────────────
#define SCAN_INTERVAL_MS 5000 // ms between scans
...
void loop() {
  ...
  // Send to tag Beetle C6 — single newline-terminated line
  ...
  // Debug
  Serial.printf("[C6] sent %d bytes to tag Beetle C6\n", json.length());
  if (json.length() > 100) {
